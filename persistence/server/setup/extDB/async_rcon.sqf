...
_key = "extDB" callExtension format["%1:%2:%3",_mode, (call A3W_extDB_rconID), _queryStmt];
...
while{_loop} do
{
	...
	else
	{
		...
		{
			diag_log format ["[extDB] Sleep [4]: %1", diag_tickTime]; // Helps highlight if someone Queries are running slow
			sleep 0.1;
		} else {
			_loop = false;
		};
