...
_key = "extDB" callExtension format["%1:%2:%3",_mode, (call A3W_extDB_steamID), _queryStmt];
...
while{_loop} do
{
	...
	else
	{
		...
		{
			diag_log format ["[extDB] Sleep [4]: %1", diag_tickTime]; // Helps highlight if Queries are running slow (steam queries will get aborted after 30 secs)
			sleep 0.1;
		} else {
			_loop = false;
		};
...
if ((_queryResult select 0) isEqualTo 0) exitWith {diag_log format ["extDB: Protocol Error: %1", _queryResult]; []};
